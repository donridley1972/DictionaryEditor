   ...
   MEMBER('DictionaryEditor.clw')                          ! This is a MEMBER module
                     ...
                     MAP
                       INCLUDE('DICTIONARYEDITOR019.INC'),ONCE        !Local module procedure declarations
                     END
...
Loc:DefaultButton     long

  CODE
  Loc:DefaultButton = BUTTON:Abort
  ...
  if ThisMessageBox.GetGlobalSetting('TranslationFile') <> ''
    returned = ds_Message(Loc:StopText,getini('MessageBox_Text','StopHeader','Stop',ThisMessageBox.GetGlobalSetting('TranslationFile')),ICON:Hand,BUTTON:Abort+BUTTON:Ignore,Loc:DefaultButton)
  elsif ThisMessageBox.GetGlobalSetting('StopHeader') <> ''
    returned = ds_Message(Loc:StopText,ThisMessageBox.GetGlobalSetting('StopHeader'),ICON:Hand,BUTTON:Abort+BUTTON:Ignore,Loc:DefaultButton)
  else
    returned = ds_Message(Loc:StopText,'Stop',ICON:Hand,BUTTON:Abort+BUTTON:Ignore,Loc:DefaultButton)
  end
